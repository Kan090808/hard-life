Create a single horizontal sprite strip for the Codex app digital pet `doraemon` in the state `jumping`.
...
Output exactly 5 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Signature Doraemon-inspired pet: blue robotic cat, white face/belly, red nose, yellow bell collar, no ears, compact proportions, friendly expression..
