...
export const JOBS = {
  ...
  1: {
    ...
    effects: { money: 760, energy: -14, stress: 7 },
  },
  2: {
    level: 2,
    name: "辦公室約聘人員",
...
export const ACTIONS = {
  work: { id: "work", label: "去上班", tone: "income", icon: "briefcase", periods: ["morning", "afternoon"], outcomes: { good: { effects: { money: 160 }, text: "今天事情特別順，還多拿到一點獎金。" }, bad: { effects: { energy: -4, stress: 7 }, text: "臨時工作全堆過來，這班比預期更累。" } } },
  ...
  jobSearch: { id: "jobSearch", label: "上 104 投履歷", tone: "growth", icon: "search", periods: ["morning"], outcomeRateModifier: -0.2, effects: { energy: -3, stress: 6 }, outcomes: { good: { effects: { stress: -3 }, text: "履歷很快得到回覆，你拿到更穩定的工作機會。", promoteJob: true }, bad: { effects: { stress: 6 }, text: "履歷被讀過，這次仍然沒有收到錄取。" } } },
  gig: { id: "gig", label: "去跑 foodpanda 外送", tone: "income", icon: "scooter", periods: ["afternoon"], effects: { money: 600, energy: -16, stress: 6 }, outcomes: { good: { effects: { money: 150, stress: -2 }, text: "剛好遇到加成時段，幾張單都很順路。" }, bad: { effects: { money: -150, energy: -4, stress: 5 }, text: "等單和找地址花掉大半時間，收入比預期少。" } } },
  tempWork: { id: "tempWork", label: "去打工", tone: "income", icon: "store", periods: ["afternoon"], effects: { money: 650, energy: -20, stress: 9 }, outcomes: { good: { effects: { money: 150, stress: -3 }, text: "現場提早收工，主辦還多補了一筆車馬費。" }, bad: { effects: { money: -150, energy: -5, stress: 6 }, text: "工作內容臨時加碼，薪水卻沒有跟著增加。" } } },
  ...
  laundry: { id: "laundry", label: "去自助洗衣", tone: "utility", icon: "tools", periods: ["evening"], effects: { money: -80, energy: -5 }, outcomes: { good: { effects: {}, text: "洗衣店空空的，折好衣服還有一小時可以處理別的事。" }, bad: { effects: { money: -40, energy: -4, stress: 4 }, text: "排隊等機器等了很久，出來天色都暗了。" } } },
  groceries: { id: "groceries", label: "去寶雅買日用品", tone: "utility", icon: "store", periods: ["evening"], effects: { money: -200, energy: -4 }, outcomes: { good: { effects: { money: -60 }, text: "剛好遇到即期品特價，省下一點錢。" }, bad: { effects: { money: -80, stress: 4 }, text: "沒注意標價，結帳才發現比預期貴了不少。" } } },
  study: { id: "study", label: "線上課程", tone: "growth", icon: "brain", periods: ["evening"], effects: { money: -260, energy: -10, stress: 5, skill: 8 }, outcomes: { good: { effects: { skill: 5 }, text: "卡很久的觀念突然想通，進度超過預期。" }, bad: { effects: { skill: -4, energy: -3, stress: 4 }, text: "精神一直飄走，花了時間卻只記住一點。" } } },
  run: { id: "run", label: "跑步", tone: "recovery", icon: "bolt", periods: ["evening"], effects: { energy: -25, stress: -14 }, outcomes: { good: { effects: { stress: -5 }, text: "跑完流了一身汗，那些煩躁好像也一起排掉了。" }, bad: { effects: { energy: -5, stress: 3 }, text: "跑沒多久就開始喘，今天的狀態不太對。" } } },
  ...
  meditate: { id: "meditate", label: "冥想", tone: "recovery", icon: "spark", periods: ["evening"], effects: {}, outcomes: { good: { effects: { stress: -12 }, text: "把煩惱暫時放下，內心寧靜了許多。" }, bad: { effects: {}, text: "無法靜下心來，只好去睡覺。" } } },
  rest: { id: "rest", label: "早點休息", tone: "recovery", icon: "bed", periods: ["evening"], effects: { energy: 18, stress: -12 }, outcomes: { good: { effects: { energy: 7, stress: -3 }, text: "難得睡得又深又安穩，整個人恢復不少。" }, bad: { effects: { energy: -8, stress: 5 }, text: "施工聲和機車聲輪流響，這一覺沒有睡好。" } } },
  ...
  network: { id: "network", label: "約朋友喝茶", tone: "growth", icon: "network", periods: ["evening"], effects: { money: -200, energy: -6, stress: -4, skill: 3 }, outcomes: { good: { effects: { stress: -3 }, text: "朋友真的知道一個小案子，答應先幫你留意。", createLead: true }, bad: { effects: { stress: 6 }, text: "聊了很久只有客套話，反而多了一點人情壓力。" } } },
  freelance: { id: "freelance", label: "接一個臨時小案", tone: "income", icon: "laptop", periods: ["evening"], effects: { money: 900, energy: -18, stress: 11 }, outcomes: { good: { effects: { money: 260 }, text: "客戶一次就確認，還主動加了一點急件費。" }, bad: { effects: { money: -250, energy: -5, stress: 8 }, text: "客戶反覆修改，尾款也被砍了一截。" } } },
  repairScooter: { id: "repairScooter", label: "牽去機車行修理", tone: "utility", icon: "scooter", periods: ["afternoon"], effects: { money: -800 }, outcomes: { good: { effects: { money: 180 }, text: "老闆只換必要零件，費用比估價低。" }, bad: { effects: { money: -180, stress: 5 }, text: "拆開後又發現一個磨損零件，只能一起處理。" } } },
  repairComputer: { id: "repairComputer", label: "去光華商場修電腦", tone: "utility", icon: "monitor", periods: ["afternoon"], effects: { money: -1000 }, outcomes: { good: { effects: { money: 220 }, text: "只是接點鬆脫，店家退回一部分預收費。" }, bad: { effects: { money: -220, stress: 5 }, text: "檢測後需要多換一個零件，修理費又增加。" } } },
...
export const RANDOM_EVENTS = [
  ...
  },
  {
    ...
    options: [
      { id: "event:phone-screen:repair", label: "去通訊行換螢幕", icon: "phone", tone: "utility", effects: { money: -1600 }, result: "螢幕恢復正常，存款卻少了一大截。" },
      { id: "event:phone-screen:endure", label: "先貼膠帶繼續用", icon: "tools", tone: "recovery", effects: { energy: -5, stress: 32 }, result: "每次點錯畫面都讓你更煩躁，但今天不用花錢。" },
    ],
  },
  {
    id: "wedding-envelope",
